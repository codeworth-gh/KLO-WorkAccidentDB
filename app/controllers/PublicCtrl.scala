...
import models.{Column, CountByCategoryAndYear, ExecutorCountRow, InjuredWorker, InjuredWorkerRow, SafetyViolationSanction, Severity, WorkAccidentSummary}
...
class PublicCtrl @Inject()(cc: ControllerComponents, accidents:WorkAccidentDAO, regions:RegionsDAO,
  ...
  def bizEntDetails(id:Long ) = Action.async{ implicit req =>
    for {
      ...
      violationSanctions <- svsDAO.getForBusinessEntity(id)
    } yield {
      bizEntOpt match {
        case None => {
          val msgs = request2Messages(req)
        ...
        case Some(bizEnt) => Ok( views.html.publicside.bizEntDetails(bizEnt, accidents, sanctions, violationSanctions))
      }
    }
  }
  ...
  def safetyWarrantsIndex() = cached( _ => PublicCtrl.SW_INDEX_PAGE_CACHE_KEY, 120) {
    Action.async{ implicit req =>
      for {
        ...
        datelessCount <- safetyWarrants.countDatelessWarrants()
      } yield {
        ...
        val years = byCatYear.flatMap(_.year).distinct.sorted
        val countPerYearByCat = byCatYear.filter( r=>r.year.nonEmpty)
                                          .map( r => (r.category.getOrElse(""), r.year.get)->r.count ).toMap
        Ok( views.html.publicside.safetywarrants.index(total, totalUnknown, top5AllTime,
          top5in24Mo, byCatAll, byCat24Mo, byLaw, years, countPerYearByCat, dateRange, datelessCount) )
      }
    }
  }
